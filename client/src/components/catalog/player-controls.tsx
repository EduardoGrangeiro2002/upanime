...
export function PlayerControls({
  ...
  const visible = useAutoHide(rootRef, playing && !menuOpen)
  ...
  return (
    ...
    >
      ...
      <div className="bg-gradient-to-t from-black/90 to-transparent px-3 pb-2 pt-8">
        <TimeSlider.Root className="group relative flex h-6 w-full cursor-pointer items-center">
          <TimeSlider.Track className="relative h-1 w-full rounded bg-white/30">
            <TimeSlider.TrackFill className="absolute h-full rounded bg-primary" style={{ width: "var(--slider-fill)" }} />
          </TimeSlider.Track>
          <TimeSlider.Thumb
        ...
        <div className="flex items-center gap-1 text-white">
          ...
          <div className="hidden items-center md:flex">
            ...
            <VolumeSlider.Root className="group relative flex h-6 w-20 cursor-pointer items-center">
              <VolumeSlider.Track className="relative h-1 w-full rounded bg-white/30">
                <VolumeSlider.TrackFill className="absolute h-full rounded bg-primary" style={{ width: "var(--slider-fill)" }} />
              </VolumeSlider.Track>
              <VolumeSlider.Thumb
                className="absolute h-3 w-3 -translate-x-1/2 rounded-full bg-primary"
                style={{ left: "var(--slider-fill)" }}
              />
            </VolumeSlider.Root>
          </div>
...
function QualityMenu({ qualities, activeQuality, onSelect, open, onOpenChange }: QualityMenuProps) {
  const rootRef = useRef<HTMLDivElement>(null)
...
function useAutoHide(containerRef: React.RefObject<HTMLElement | null>, canHide: boolean): boolean {
